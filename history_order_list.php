...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_order'])) {
    ...
    try {
        ...
        if ($order) {
            ...
            $order_date = $order['order_date']; // Sipariş tarihini al
            ...
            // Siparişi sil
            $stmt_delete_order = $conn->prepare("DELETE FROM orders WHERE order_id = :order_id");
            $stmt_delete_order->bindParam(':order_id', $order_id, PDO::PARAM_INT);
            if ($stmt_delete_order->execute()) {
                // Silme işlemi başarılıysa, aynı sayfayı yenile
            ...
            <tr>
                ...
                <td>
                    <form method="post">
                        ...
                        <button type="submit" name="delete_order" class="btn btn-danger">Sil</button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
